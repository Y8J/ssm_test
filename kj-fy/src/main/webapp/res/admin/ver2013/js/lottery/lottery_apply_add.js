...
for (var i = 0; i < js.length; i++) {  
    if (js[i].src.indexOf("lottery_apply_add.js") >= 0) {  
...
$(function() {
	...
	var params="?model="+$('input[name="model"]:checked').val()+"&pageStyle="+$('input[name="pageStyle"]:checked').val()+"&topBg="+$("#uploadImgPath1").val()+"&frameBg="+$("#uploadImgPath2").val()+"&guideBg="+$("#uploadImgPath3").val()+"&dialBg="+$("#uploadImgPath4").val();
	$("#previewFrame")[0].src="v_preView.do"+params;
	hideFrameModal();
	if ($.browser.msie) {
	...
	$("input[name='dateModel']").click(function(e){
		...
		if(chVal==1){
			$(".date_range_item:eq(0)").siblings().remove();
			$("#div_date_more").hide();
		}else{
			$("#div_date_more").show();
		}
	...
	$("#btnSave").click(function(e){
		...
		$("#rules").html(_rules);
		$(loading).modal();
		$.post("o_add.do",$("#jvForm").serialize(),function(data){
			$.modal.close();
			alert(data.m);
			if(data.c==1){
				window.location.href=window.location.href;
			}else if(typeof(data.focusEl)!="undefined"){
				$("#"+data.focusEl).focus();
			}else if(typeof(data.focusElName)!="undefined"&&typeof(data.focusIndex)!="undefined"){
				$('input[name="'+data.focusElName+'"]')[data.focusIndex].focus();
			}
